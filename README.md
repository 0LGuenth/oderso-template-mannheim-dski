...
- **Statutory declaration wording**: `use-old-statutory-declaration` (`dhbw-ka` and `dhbw-ma`) defaults to `true` and matches the DSKI LaTeX declaration, including the work title. Set it to `auto` to retain the prior course-year detection or `false` to use the current wording.
